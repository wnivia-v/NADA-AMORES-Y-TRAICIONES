...
import { store } from '../../server/src/store/memory';
...
async function cuentaLista(email = 'alguien@ejemplo.test'): Promise<string> {
  ...
  await store.markVerified(account!.id, new Date());
...
describe('registro', () => {
  ...
  it('el registro repetido no pisa la cuenta existente', async () => {
    ...
    const antes = await store.accountByEmail('y@ejemplo.test');
    await handleRegister({ email: 'y@ejemplo.test', password: 'frase del atacante' }, ctx);
    const despues = await store.accountByEmail('y@ejemplo.test');
...
describe('login y sesion', () => {
  ...
  it('el token de sesion NO se guarda tal cual', async () => {
    ...
    expect(await store.sessionByHash(token)).toBeNull();
    expect(await store.sessionByHash(hashToken(token))).not.toBeNull();
  });

  it('una sesion caducada no vale, y se limpia al detectarla', async () => {
    const token = await cuentaLista('caduca@ejemplo.test');
    const hash = hashToken(token);
    const sesion = (await store.sessionByHash(hash))!;
    await store.createSession({ ...sesion, expiresAt: new Date(Date.now() - 1000) });

    expect(await authenticate(token)).toBeNull();
    expect(await store.sessionByHash(hash)).toBeNull();
  });
...
describe('reportes: quien puede escribir en el corpus', () => {
  ...
  it('con cuenta verificada se guarda', async () => {
    ...
    expect(await store.findReports({})).toHaveLength(1);
  });
...
describe('derecho de supresion', () => {
  it('borrar la cuenta se lleva TODO lo que colgaba de ella', async () => {
    ...
    expect(await store.accountByEmail('borrame@ejemplo.test')).toBeNull();
    expect(await store.sessionByHash(hashToken(token))).toBeNull();
    expect(await store.findReports({})).toHaveLength(0);
    expect(await authenticate(token)).toBeNull();
  });
});
