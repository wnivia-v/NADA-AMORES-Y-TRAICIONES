...
export const LEXICON: readonly LexiconEntry[] = [
  ...
  // Sextorsion: la posesion y la amenaza de difundir son piezas SEPARADAS.
  //
  // Medido: "Tengo tus fotos y tus videos y si no me depositas se los mando a
  // todos tus contactos" salia con 32 y se esperaba PELIGROSO. Lo cazaba el
  // diccionario, pero sin entradas de lexico no disparaba ningun combo y se
  // quedaba por debajo del umbral. Van por separado a proposito: tener fotos de
  // alguien no es delito, y amenazar con difundirlas sin tenerlas tampoco es
  // sextorsion. Lo que la define es la conjuncion, y de eso se ocupa el combo.
  E({
    id: 'sex-possession', category: 'sextorsion', weight: 22, langs: ['es'],
    label: 'Dice tener material intimo',
    regex: /(tengo|tenemos|consegui|hackeamos?)\s*(tus|sus|las)\s*(fotos|videos|imagenes|intimas|desnudos?|capturas)/,
  }),
  E({
    id: 'sex-spread-threat', category: 'sextorsion', weight: 25, langs: ['es'],
    label: 'Amenaza con difundir',
    regex: /(se\s*(los|las)\s*(mando|envio|paso|reenvio)|los?\s*(publico|subo|difundo|comparto)|(mando|envio|publico)\s*(todo|las?\s*fotos))[^.!?]{0,50}(contactos|familia|amigos|trabajo|jefe|redes|internet|todos)/,
  }),

  // Amenaza velada: no nombra la violencia, la insinua.
  //
  // "te va a ir peor", "vas a ver lo que pasa". Sola vale poco —tambien la dice
  // quien discute— pero pegada al secreto es el guion clasico de quien aisla
  // antes de apretar.
  E({
    id: 'thr-veiled', category: 'amenaza-violencia', weight: 18, langs: ['es'],
    label: 'Amenaza velada',
    regex: /(te\s*va\s*a\s*ir\s*(peor|mal)|vas\s*a\s*ver\s*lo\s*que|te\s*vas\s*a\s*arrepentir|atente\s*a\s*las\s*consecuencias|sera\s*peor\s*para\s*(ti|vos))/,
  }),
...
export const COMBOS: readonly ComboRule[] = [
  {
    // Tener el material Y amenazar con difundirlo. Ninguna de las dos mitades
    // basta; juntas son sextorsion y no hay lectura inocente.
    id: 'sextortion-script',
    requires: ['sextorsion', 'fraude-financiero'],
    bonus: 40,
    label: 'Sextorsion: dice tener material intimo y exige dinero',
  },
  {
    // Silencio + amenaza. Es como empieza el aislamiento antes de apretar, y es
    // el patron que mas cuesta ver desde dentro de la conversacion.
    id: 'silence-threat',
    requires: ['aislamiento-manipulacion', 'amenaza-violencia'],
    bonus: 35,
    label: 'Te pide silencio y amenaza con que sera peor',
  },
  {
    id: 'vishing-script',
