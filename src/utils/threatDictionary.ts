 ...
 */
/**
 * Terminos que por si solos no acusan de nada.
 *
 * Son palabras del diccionario que aparecen igual en una estafa que en una
 * conversacion corriente: "dinero", "te amo", "mi amor". Puntuaban solas, y
 * asi un mensaje como
 *
 *   "Mi amor, ya pague la cuenta de la luz y transferi el dinero del alquiler"
 *
 * salia con 47/100 — un caso del corpus que ANTES pasaba y que esta lista
 * devuelve a su sitio.
 *
 * Es exactamente el Problema A del brief: «el diccionario evalua palabras
 * aisladas, no contexto». La regla que se aplica es la que ya usa el resto del
 * proyecto: cuentan cuando algo mas de su categoria las respalda, nunca solas.
 */
const AMBIGUOS_SIN_CONTEXTO = new Set([
  // Dinero: nombrarlo no es pedirlo.
  'dinero', 'plata', 'lana', 'feria', 'varo', 'varos', 'efectivo', 'pago',
  // Afecto: es lo que se dicen las parejas, y este producto existe para
...
function tieneRespaldo(palabras: string[]): boolean {
  return palabras.some((w) => !AMBIGUOS_SIN_CONTEXTO.has(w));
}
...
export function scanDictionary(text: string): DictScanResult {
  ...
  /** Que palabras sostienen cada categoria, para poder exigirles respaldo. */
  const palabrasPorCategoria = new Map<DictCategory, string[]>();
  const anotar = (cat: DictCategory, palabra: string) => {
    const lista = palabrasPorCategoria.get(cat) ?? [];
    lista.push(palabra);
    palabrasPorCategoria.set(cat, lista);
  };
  ...
  for (const entry of MULTI_WORD_PHRASES) {
    ...
    if (current < max) {
      hitCounts.set(entry.category, current + 1);
      anotar(entry.category, entry.phrase);
  ...
  for (const [catId, hits] of hitCounts) {
    ...
    // Una categoria sostenida SOLO por terminos ambiguos no puntua. No se
    // descarta la palabra: se descarta que baste ella sola para acusar.
    if (!tieneRespaldo(palabrasPorCategoria.get(catId) ?? [])) continue;
